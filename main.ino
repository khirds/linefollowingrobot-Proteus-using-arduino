...
void sdint()
{
 Serial.print("Initializing SD card...");
  ...
  // if the file opened okay, write to it:
  if (myFile) {
    Serial.print("Writing to DAQ.txt...");
    myFile.println("testing 1, 2, 3,4 test.");
	// close the file:
    myFile.close();
    Serial.println("done.");
  } else {
    // if the file didn't open, print an error:
    Serial.println("error opening DAQ.txt");
  }
  
  // re-open the file for reading:
  myFile = SD.open("DAQ.txt");
  if (myFile) {
    Serial.println("DAQ.txt:");
    ...
    myFile.close();
  	...
  	// if the file didn't open, print an error:
    Serial.println("error opening DAQ.txt.txt");
  }
 }
